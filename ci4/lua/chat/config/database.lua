...
-- Database configuration
_M.config = {
    host = env_vars["database.default.hostname"] or os.getenv("DB_HOST") or "workerra-ci-db",
    port = tonumber(env_vars["database.default.port"] or os.getenv("DB_PORT") or "3306"),
    database = env_vars["database.default.database"] or os.getenv("DB_DATABASE") or "myworkstation_dev",
    user = env_vars["database.default.username"] or os.getenv("DB_USERNAME") or "workerra-ci-dev",
    password = env_vars["database.default.password"] or os.getenv("DB_PASSWORD") or "CHANGE_ME",
    charset = "utf8mb4",
